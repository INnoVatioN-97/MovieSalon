...
export const getTmdbBoxOffice = async () => {
  // TMDB_박스오피스 => Home.js
  try {
...
export const getHotWeekMovies = async () => {
  // 1주일 기준 BoxOffice 상위 rank => Home.js
  try {
    const TMDB_API_KEY = process.env.REACT_APP_TMDB_API_KEY;
    const {
      data: { results },
...
export const getTMDBSearchKRBoxOffice = async (keywords) => {
  try {
    const TMDB_API_KEY = process.env.REACT_APP_TMDB_API_KEY;
    const {
      data: { results },
    ...
    // console.log('error_gettmdbsearchKr!', error);
  }
};
